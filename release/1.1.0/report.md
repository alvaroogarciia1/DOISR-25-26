...
1. `dsh:DigimonCoreShape`  
   Checks the basic required information for each Digimon, such as label, stage/type subclass, attribute and memory.
...
| `dig:Brokenmon` does not belong to any Digimon type subclass | `sh:or` | Type it as one of the valid subclasses, for example `dig:VirusDigimon` |
...
| Invalid move type: `dig:Fire` is an `Attribute`, not a `MoveType` | `sh:class dig:MoveType` | Use a valid move type, such as `dig:Physical` or `dig:Magic` |
...
After updating the ontology conceptualization, the SHACL shapes were adapted to the new modeling decision where Digimon types are represented as subclasses of `dig:Digimon` instead of using the previous `dig:hasType` property and `dig:DigimonType` class.
